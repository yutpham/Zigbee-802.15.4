...
#if OS_ZIGBEE_FRW == 1
#define  OSTmr_Lock()     OS_ENTER_CRITICAL()
#define  OSTmr_Unlock()   OS_EXIT_CRITICAL()
#endif
...
#if OS_TMR_EN > 0 && OS_ZIGBEE_FRW == 0
...
#endif
#if OS_ZIGBEE_FRW == 0                                    /* For Zigbee do not need it, must be tested first */  
    if (ptmr->OSTmrType != OS_TMR_TYPE) {                   /* Validate timer structure                               */
        *perr = OS_ERR_TMR_INVALID_TYPE;
        return (OS_FALSE);
    }
...
static  void  OSTmr_Free (OS_TMR *ptmr)
{
    ...
    ptmr->OSTmrPrev        = (OS_TMR *)0;              /* Chain timer to free list                                    */
